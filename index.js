...
let datumMesic;
let denVtydnu;
const dnyCesky = ['Neděle', 'Pondělí', 'Úterý', 'Středa', 'Čtvrtek', 'Pátek', 'Sobota'];
let fetchCesta = 'https://api.openweathermap.org/data/2.5/onecall?lat=48.8957&lon=17.3180&exclude=minutely,hourly&units=metric&lang=cz&appid=55e696532f166939726049f05dfb145d';
...
function zobrazPocasi (data){
	...
	document.querySelector ('#teplota').textContent = Math.round(data.current.temp);
	document.querySelector ('#popis').textContent = data.current.weather[0].description;
	document.querySelector ('#vlhkost').textContent = data.current.humidity;
	document.querySelector ('#vitr').textContent = data.current.wind_speed.toFixed(1);
	...
	//ikony
	let novaIkona = getWeatherIcon(data.current.weather[0].id, data.current.weather[0].icon);
	...
	for (let i=1; i<5; i++) {
		...
		let ikona = getWeatherIcon(data.daily[i].weather[0].id, data.daily[i].weather[0].icon);
		
		predpovedHtml += `<div class="forecast">
		<div class="forecast__day">${dnyCesky[denVtydnu]} ${datumKolikateho}.${datumMesic+1}.</div>
...
function Praha () {
	mesto = 'Praha';
	fetchCesta = 'https://api.openweathermap.org/data/2.5/onecall?lat=50.0847&lon=14.4215&exclude=minutely,hourly&units=metric&lang=cz&appid=55e696532f166939726049f05dfb145d';
	nactiMisto();
...
function Brno () {
	mesto = 'Brno';
	fetchCesta = 'https://api.openweathermap.org/data/2.5/onecall?lat=49.20&lon=16.61&exclude=minutely,hourly&units=metric&lang=cz&appid=55e696532f166939726049f05dfb145d';
	nactiMisto();
	const pozadi = document.querySelector ('body');
...
function Ostrava () {
	mesto = 'Ostrava';
	fetchCesta = 'https://api.openweathermap.org/data/2.5/onecall?lat=49.83&lon=18.26&exclude=minutely,hourly&units=metric&lang=cz&appid=55e696532f166939726049f05dfb145d';
	nactiMisto();
	const pozadi = document.querySelector ('body');
...
function domu () {
	mesto = 'Strážnice';
	fetchCesta = 'https://api.openweathermap.org/data/2.5/onecall?lat=48.8957&lon=17.3180&exclude=minutely,hourly&units=metric&lang=cz&appid=55e696532f166939726049f05dfb145d';
	nactiMisto();
	const pozadi = document.querySelector ('body');
